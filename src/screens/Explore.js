...
import {View, Text, Dimensions, TouchableOpacity, FlatList,
  StyleSheet, ImageBackground, BackHandler} from 'react-native';
...
class Explore extends React.PureComponent{
  ...
  renderHeader(){
    ...
    return (
      <View style={{margin:8, height:70, justifyContent:'space-between',
        alignItems:'center', flexDirection:'row', paddingHorizontal:25, paddingVertical:10}}>
        <TouchableOpacity
          activeOpacity={1}
          onPress={() => {this.props.clearSearch();Actions.pop()}}
  ...
  renderBigImage(){
    ...
    return (
      <View style={{flex:1, justifyContent:'space-around'}}>
        <View style={{width:screenWidth-20, height:120, overflow:'hidden',
  ...
  renderArticle({item}){
    ...
    return(
      <View style={{width:(screenWidth-40), alignItems:'center', marginBottom:10}}>
        <ArticleTile
          data={item}
    ...
    return (
      <View style={{flex:1, justifyContent:'center', alignItems:'center'}}>
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={{fontFamily:FONTS.PRODUCT_SANS, color:COLORS.DARK, fontSize:18}}>
